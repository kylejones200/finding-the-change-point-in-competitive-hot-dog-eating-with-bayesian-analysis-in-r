...
def simulate_change_point_data(n: int=100, change_point: int=50, before_mean: float=10, after_mean: float=20, std: float=2, seed: int=42) -> pd.DataFrame:
    ...
    return pd.DataFrame({'time': np.arange(n), 'value': data})
...
def plot_change_point_detection(df: pd.DataFrame, detected_cp: int, title: str, output_path: Path, plot: bool=False):
    """Plot change point detection"""
    if not plot:
        return
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(df['time'], df['value'], color='#4A90A4', linewidth=1.2)
    ax.axvline(detected_cp, color='red', linestyle='--', linewidth=1.5, label=f'Detected Change Point: {detected_cp}')
    ax.set_xlabel('Time')
    ax.set_ylabel('Value')
    ax.legend(loc='best')
